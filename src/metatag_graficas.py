...
def show_stats(parent, df, img_col_var, current_scale, C, FONTS):
    ...
    win.minsize(int(800 * current_scale), int(500 * current_scale))
    ...
    def make_selector(parent, label_text, icon, options, default, on_change):
        ...
        def toggle(e=None):
            ...
            if x + popup_w > screen_w - 10:
                x = max(4, screen_w - popup_w - 10)
            if y + popup_h > screen_h - 10:
                y = max(4, screen_h - popup_h - 10)
    ...
    def export_chart():
        ...
        if not path:
            return
        try:
            canvas = canvas_widget.get_tk_widget()
            orig_size = fig.get_size_inches()
            configure_binding = canvas.bind("<Configure>")
            canvas.unbind("<Configure>")
            try:
                fig.set_size_inches(*EXPORT_FIG_SIZE, forward=False)
                fig.savefig(path, dpi=EXPORT_DPI, facecolor=fig.get_facecolor(),
                            bbox_inches="tight", pad_inches=0.3)
            finally:
                fig.set_size_inches(*orig_size, forward=False)
                if configure_binding:
                    canvas.bind("<Configure>", configure_binding)
    ...
    insight_area = tk.Frame(info_frame, bg=S_CARD)
    insight_area.pack(fill="both", expand=True)
    insight_text = tk.Text(insight_area, bg=S_CARD, fg=S_TEXT, font=FONTS["BODY"],
                           wrap="word", relief="flat", padx=18, pady=18)
    insight_vsb = ttk.Scrollbar(insight_area, orient="vertical",
                                command=insight_text.yview)
    insight_text.configure(yscrollcommand=insight_vsb.set)
    insight_vsb.pack(side="right", fill="y")
    insight_text.pack(side="left", fill="both", expand=True)
