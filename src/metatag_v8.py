...
class MetaTagApp(tk.Tk):
    ...
    # ─────────────────────────────────────────────────────────────
    #  ESTADÍSTICAS / GRÁFICOS  (delegado a metatag_graficas.py)
    # ─────────────────────────────────────────────────────────────
    def _show_stats(self):
        _show_stats_external(self, self.df, self.img_col_var,
                             self.current_scale, C, FONTS)
    ...
    def _match_rows_to_files(self, df, img_col, all_files, by_exact_name, by_stem, folder):
        ...
        for ri in range(len(df)):
            ...
            if found_path is None:
                approx, approx_status, approx_candidates = self._find_image_ex(val, folder)
                if approx:
                    approx_path = Path(approx)
                    if approx_path not in used_files:
                        found_path = approx_path
                        matches_aprox.append((val, found_path.name))
                elif approx_status == "ambiguous":
                    matches_ambiguas.append((val, approx_candidates))
                    ambiguous_row = True

            if found_path:
                ordered_files.append(found_path)
                used_files.add(found_path)
            elif not ambiguous_row:
                no_encontradas.append(val)

        return ordered_files, matches_aprox, no_encontradas, used_files, matches_ambiguas
    ...
    def _sync_images_to_excel(self):
        ...
        ordered_files, matches_aprox, no_encontradas, used_files, matches_ambiguas = \
        ...
        if matches_ambiguas:
            self._log(
                f"  ⛔ {len(matches_ambiguas)} fila(s) AMBIGUAS: varios archivos "
                f"comparten la misma clave de fallback, NO se emparejó ninguno "
                f"automáticamente. Resuélvelas manualmente:\n", "warn")
            for excel_val, candidates in matches_ambiguas[:10]:
                self._log(f"     '{excel_val}'  →  "
                          f"{', '.join(Path(c).name for c in candidates)}\n", "warn")
            if len(matches_ambiguas) > 10:
                self._log(f"     … y {len(matches_ambiguas)-10} más.\n", "warn")
    ...
    def _find_image_ex(self, name, folder):  # -> tuple[Optional[str], str, list]
        """
        Busca la imagen asociada a 'name' en 'folder'.

        Devuelve (path, status, candidates):

        status="ok"        → path es la única correspondencia válida.
        status="not_found" → path=None, candidates=[].
        status="ambiguous" → path=None, candidates=todos los candidatos que
                             compiten por la misma clave de fallback. La
                             ambigüedad impide elegir uno arbitrariamente.
        """
        name = name.strip()
        if not name:
        ...
        if name_stem in self._img_cache:
    ...
    def _find_image(self, name, folder):  # -> Optional[str]
        path, _, _ = self._find_image_ex(name, folder)
        return path
